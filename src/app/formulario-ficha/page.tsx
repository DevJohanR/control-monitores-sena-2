...
export default function FormularioFicha() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFichas([...fichas, newFicha]); // Agregar nueva ficha al estado directamente
  ...
  const deleteFicha = async (id: number) => {
    try {
      ...
      // Actualizamos el estado eliminando la ficha del arreglo sin recargar
    ...
    try {
      ...
      setFichas((prevFichas) =>
        prevFichas.map((ficha) =>
          ficha.idFicha === id
            ? { ...ficha, numeroFicha: updatedFicha.numeroFicha } // Actualizamos solo el número de ficha que cambió
            : ficha // Mantenemos las demás fichas igual
        )
      );
    } catch (error) {
      console.error(error);
...
<>
    ...
    <div className="container mx-auto my-8 px-4 mt-20">
      ...
      <ul className="bg-white shadow-md rounded px-4 py-4">
        {fichas.map((ficha: { idFicha: number; numeroFicha: string }) => (
          <li
            key={ficha.idFicha}
